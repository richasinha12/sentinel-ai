...
export function RunPanel() {
  ...
  return (
    <div className="bg-sentinel-800 rounded-xl p-5 border border-slate-700">
      <h2 className="font-bold text-lg mb-4">🚀 Run Intelligence Sweep</h2>
      <div className="space-y-3 mb-4">
        <label className="block">
          <span className="text-xs text-slate-400 uppercase tracking-wider">🎯 Competitors (GTM)</span>
        ...
        <label className="block">
          <span className="text-xs text-slate-400 uppercase tracking-wider">📊 Customers (Finance)</span>
          <input value={customers} onChange={(e) => setCustomers(e.target.value)}
            className="mt-1 w-full bg-sentinel-900 border border-slate-600 rounded px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
        </label>
        <label className="block">
          <span className="text-xs text-slate-400 uppercase tracking-wider">🛡️ Vendors (Security)</span>
      ...
      <button onClick={handleRun} disabled={isRunning}
        ...
        {isRunning ? '⏳ Agents Running...' : '▶ Run All Agents'}
      </button>
    </div>
  )
}
